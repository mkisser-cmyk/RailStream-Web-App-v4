...
agent_communication:
  ...
  - agent: "main"
    message: "PRE-ROLL AD FIX + AUTH HEARTBEAT + ADMIN ACCESS FIX: 1) Fixed video playing behind pre-roll ad - Added adPlaying prop to HlsPlayer that pauses/mutes video when ad is active and resumes when ad ends. Also blocks autoPlay during MANIFEST_PARSED when ad is active. 2) Added auth session heartbeat - Every 5 minutes, validates token via GET /api/auth/me. If expired (401), clears auth and prompts re-login. Also updates user data (tier, admin status) from server on each heartbeat. 3) Fixed admin access check - canAccess() now accepts isAdmin flag, admins bypass all tier checks. Updated CameraPicker and CamerasPage to pass is_admin. 4) Enhanced loadCamera error handling - Detects stale session when API returns upgrade_required for locally-authorized user, triggers re-login. Also handles HTTP 401 from authorize endpoint. 5) Added NGINX X-Forwarded-For config to DEPLOYMENT.md with full proxy_pass config including Cloudflare IP ranges."